...
function FETCH_TABLE_FROM_DB($sql, $array = false)
{
    $Data = SELECT("$sql");
    $Count = CHECK("$sql");
    if ($Count == 0) {
...
//get data at details
function GET_DATA($table, $column, $conditions, $die = false)
{
    $PageSqls = "SELECT * FROM $table where $conditions";

    if ($die == true) {
        FETCH($PageSqls, $column, true);
    } else {
        $results = FETCH($PageSqls, $column);
    }
