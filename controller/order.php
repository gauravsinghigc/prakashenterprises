...
if (isset($_POST['SaveItemIntoCart'])) {
    ...
    $CartSessionId = $_SESSION['INVOICE_AND_CREATION_SESSION'];
    $FetchCartItems = FETCH_TABLE_FROM_DB("SELECT * FROM invoice_cart where InvoiceCartCustomerId='$CustomerId' and CartSessionId='$CartSessionId'", true);
    ...
    if ($cartitemsstatus == true) {
        //invoice details
        $Address = "";
        $FetchAddress = FETCH_TABLE_FROM_DB("SELECT * FROM user_addresses where UserAddressUserId='$CustomerId' ORDER BY UserAddressId DESC LIMIT 1", true);
